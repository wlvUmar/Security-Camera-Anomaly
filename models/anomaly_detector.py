...
class AnomalyDetector:
    ...
    def calculate_motion(self, current_features: Dict, frame_shape: Tuple) -> Dict:
        ...
        time_diff = current_features['frame_timestamp'] - prev_features['frame_timestamp']
    ...
    def detect_covered(self, frame: np.ndarray, base_threshold: float = 50.0) -> bool:
    ...
    def detect_anomaly(self, frame: np.ndarray) -> Dict:
        ...
        flat = [cls for frame_objs in self.anomaly_memory["unallowed_objects"] for cls in frame_objs]
        counts = Counter(flat)

        persistent_unallowed = [cls for cls, count in counts.items() if count >= 3]
        ...
        self.anomaly_memory["unusual_movement"].append(bool(motion_data["anomalous_objects"]))
        self.anomaly_memory["intense_motion"].append(motion_data["motion_intensity"] > 0.1)
        ...
        if features['objects']:
            avg_conf = np.mean([obj['confidence'] for obj in features['objects']])
        else:
            avg_conf = 0.0
